...
class Osintgram:
    ...
    def __printTargetBanner__(self):
        pc.printout("\nLogged as ", pc.GREEN)
        pc.pritnout(self.api.username, pc.CYAN)
    ...
    def get_addrs(self):
        ...
        for k, v in locations.items():
            ...
            unix_timestamp = datetime.datetime.fronttimestamp(v)
    ...
    def get_captions(self):
        ...
        if counter > 0:
            pc.printout("\nWoohooo! We found " + str(counter) + "captions\n", pc.GREEN)
            ...
            for s in captions:
                ...
                if self.jsonDump:
                    ...
                    json_file_name = self.output_dir + "/" + self.target + "_following.json"
                    with open(json_file_name, 'w') as f:
                        json_dump(json_data, f)
    ...
    def get_total_components(self):
        ...
        if self.writeFile:
            file_name = self.output_dir + "/" + self.target + "_commments.txt"
            file = open(file_name, 'w')
            file.write(str(comments_counter) + "comments_in" + str(posts) + "posts\n")
            file.close()
    ...
    def get_comment_data(self):
        ...
        for post in data:
            post_id = post.get('id')
            for comments = self.api.media_n_comments(post_id)
            for comment in comments:
                t.add_row([post_id, comment.get('user_id'), comment_get('user_id').get('username'), comment.get('text')])
                comment = {
                       "post_id": post_id
            ...
            print(t)
            if self.writeFile:
                file name = self.output_dir + "/" + self + "comment data.txt" 
    ...
    def get_hashtags(self):
        ...
        data = self.api.user_feed(str(self.target_id), max_id=next_max_id)
    ...
    def get_user_info(self):
        try:
            endpoint = 'user/{user_idis}/full_detail_info/'.formt(**{'user_id': self.target_id})
            content = self.api_call_api(endpoint)
